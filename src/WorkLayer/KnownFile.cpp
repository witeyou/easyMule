...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
    ...
    // NOTE (SXC) to fix "file->_file"
    int tmpFileHandle = (int)(HANDLE)_get_osfhandle(_fileno(file));

    // set filesize
    if (_filelengthi64(tmpFileHandle) > MAX_EMULE_FILE_SIZE)  // MODIFY BY SXC
    ...
    SetFileSize((uint64)_filelengthi64(tmpFileHandle));  // MODIFY BY SXC
    ...
    struct _stat fileinfo;
        ...
        // NOTE (SXC) to fix "file->_file"
    int tmpFileHandle2 = (int)(HANDLE)_get_osfhandle(_fileno(file));

    if (_fstat(tmpFileHandle2, &fileinfo) == 0)
    //if (_fstat(file->_file, &fileinfo) == 0)  // MODIFY BY SXC
    {
